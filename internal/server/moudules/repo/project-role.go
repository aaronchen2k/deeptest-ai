...
func (r *ProjectRoleRepo) Create(projectRole model.ProjectRole) (err error) {
	...
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		_logs.Errorf("创建项目角色失败%s", err.Error())
		return
	}
	if role.ID != 0 {
		_logs.Infof("项目角色%s已经存在", projectRole.Name)
		return
	}
...
func (r *ProjectRoleRepo) BatchCreate(projectRoles []model.ProjectRole) (err error) {
	...
	if err != nil {
		_logs.Errorf("批量创建项目角色%s失败%s", projectRoles, err.Error())
	}
...
func (r *ProjectRoleRepo) GetAllRoleNameIdMap() (data map[consts.RoleType]uint, err error) {
	...
	if err != nil {
		_logs.Errorf("get all role list err ", zap.String("错误:", err.Error()))
		return
	}
